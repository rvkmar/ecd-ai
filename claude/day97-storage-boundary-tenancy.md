...
NODE_OPTIONS=--max-old-space-size=3072 npx vitest run
  1688 passed | 9 skipped (1697) — 143 files (~165s)

npm run build
  green (close re-run ~3m; max JS chunk recharts 451.37 kB)
```

- No half-applied product work. Commit **`272644d`** (product + session close).
- Exit check executed: storage helpers + live `:6060` after node rebuild (foreign district report 403).
- Calendar: **ECD D97** marked ✅ (event date unchanged).
- Remote `main/master` at **`272644d`**.
