...
| Last completed unit | **D97** — Storage-boundary tenancy (`loadDB` + `dbAdapter`). Handoff: `claude/day97-storage-boundary-tenancy.md`. |
...
| Next queued | Calendar **D98** — row-level negative tests (never-compress). |

| Block | **W20** — Multi-tenancy (D96–D97 done; D98–D100 open). Gate: district cannot read another district’s data, proven by test. |
...
| 2026-09-19 | D97 | 3, alone | Storage-boundary tenancy: ALS + loadDB/dbAdapter filters + merge-on-save; path-param district 403. Suite **1688 passed / 9 skipped**. Live `:6060` after node rebuild. |
...
| D97 | Enforce in/above `dbAdapter` only | Most tenant reads use `loadDB`; save without merge would wipe tenants | Shared ALS filter on loadDB + dbAdapter; mergeScopedDbSave; path-param guard |
...
- ~~Session ownership-scoping (a student may only read their own session)~~ closed D72 on session + session-report routes; ~~`GET /api/students` roster dump~~ closed D97 at storage boundary (row-level matrix still D98)
