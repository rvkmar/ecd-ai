...
export function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, async (err, user) => {
    ...
    // D97: bind ALS before next() so loadDB / dbAdapter see the claim.
    // resolveAuthEpoch above runs without ALS on purpose (unscoped user lookup).
    // enterWith keeps async route continuations in scope; run() wraps next().
    const tenancy = tenancyContextFromReq(req);
    bindTenancyContext(tenancy);
    runWithTenancy(tenancy, () => next());
  });
}
